            ...
            <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2 img-row">
                ...
                <div class="caption text-center">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu sin_permisos" role="menu">
                            <li><a class="actualizar" href="javascript:rename('{{ $item_name }}')"><i
                                        class="fa fa-edit fa-fw"></i> {{ Lang::get('laravel-filemanager::lfm.menu-rename') }}
                                </a></li>
                            @if($item->is_file)
                                <li><a class="actualizar" href="javascript:download('{{ $item_name }}')"><i
                                ...
                                @if($thumb_src)
                                    ...
                                    <li><a class="actualizar" href="javascript:resizeImage('{{ $item_name }}')"><i
                                                class="fa fa-arrows fa-fw"></i> {{ Lang::get('laravel-filemanager::lfm.menu-resize') }}
                                        </a></li>
                                    <li><a class="actualizar" href="javascript:cropImage('{{ $item_name }}')"><i
                                                class="fa fa-crop fa-fw"></i> {{ Lang::get('laravel-filemanager::lfm.menu-crop') }}
                                        </a></li>
                                    <li class="divider"></li>
                                @endif
                            @endif
                            <li><a class="eliminar" href="javascript:trash('{{ $item_name }}')"><i
...
<script>
    $.get("admin/list/privilegios_biblioteca", function (data, status) {
        if (!data.is_actualizar && !data.is_eliminar) $('.sin_permisos').addClass('hide');
        if (!data.is_actualizar) $('.actualizar').addClass('hide');
        if (!data.is_eliminar) $('.eliminar').addClass('hide');
    });
</script>
